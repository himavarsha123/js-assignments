...
function roundToPowerOfTen(num, pow) {
    ...
    var numToRound = num.slice(num.length - (pow + 1));
    num = num.slice(0, num.length - (pow + 1));
    if(parseInt(numToRound.charAt(1)) > 5) {
        numToRound = parseInt(numToRound.charAt(0)) + 1 + "0".repeat(numToRound.length - 1);
    } else {
        numToRound = numToRound.charAt(0) + "0".repeat(numToRound.length - 1);
    }
    return parseInt(num + numToRound);
}
